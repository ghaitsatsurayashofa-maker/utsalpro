public class Variabel{
    public static void main(String[] args){
        String nama = "Ghaitsa Tsuraya Shofa";
        String tempatTanggalLahir = "Sukabumi, 09 Agustus 2006";
        String jenisKelamin = "Perempuan";
        String alamat = "Cicurug, Desa Benda";
        String noHp = "0821637282";
        double penghasilanSebulan = 2900000;

        System.out.println("Assalamu'alaikum");
        System.out.println("Izinkan saya memperkenalkan diri");
        System.out.println("Nama Lengkap :" + nama);
        System.out.println("Tempat dan Tanggal Lahir :" + tempatTanggalLahir);
        System.out.println("Jenis Kelamin :" + jenisKelamin);
        System.out.println("Alamat Lengkap :" + alamat);
        System.out.println("No Hp :" + noHp);
        System.out.println("Penghasilan Sebulan :Rp" + penghasilanSebulan);

    }
}